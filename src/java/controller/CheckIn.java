...
public class CheckIn extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Customer customerForCheckIn = customerFacade.findCustomer(username);
        String tempDate = null;
        int tempNumberOfDays = 0;

        try (PrintWriter out = response.getWriter()) {
            /* TODO output your page here. You may use following sample code. */
            if (customerForCheckIn != null) {
                for (int i = 0; i < allBooking.size(); i++) {
                    if (allBooking.get(i).getCustomer().equals(customerForCheckIn)) {
                ...
                if (customerBookingFound.size() == 0) {
                    ...
                    if ((customerBookingFound.get(0).isCheckIn()) == false && (customerBookingFound.get(0).isDeletedFlag()) == true) {
                        tempNumberOfDays = customerBookingFound.size();
                        tempDate = customerBookingFound.get(0).getBookingDate();
                    } else {
                        out.print("User already checked in");
                        request.getRequestDispatcher("Home.jsp").include(request, response);
                    }

                }
                session.setAttribute("customerBookingFound", customerBookingFound);
                session.setAttribute("customerForCheckIn", customerForCheckIn);
                session.setAttribute("tempNumberOfDays", tempNumberOfDays);
                session.setAttribute("tempDate", tempDate);
                response.sendRedirect("/EPDA-war/CheckInProcess.jsp");
