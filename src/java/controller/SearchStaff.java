 ...
 * @author danen
 */
@WebServlet(urlPatterns = {"/SearchStaff"})
public class SearchStaff extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Staff staffToEdit = staffFacade.findStaff(username);
        Staff staff = (Staff) session.getAttribute("staff");

        try (PrintWriter out = response.getWriter()) {
            /* TODO output your page here. You may use following sample code. */
            if (staffToEdit != null) {
                response.sendRedirect("/EPDA-war/EditStaff.jsp");
                session.setAttribute("staffToEdit", staffToEdit);
            } else {
