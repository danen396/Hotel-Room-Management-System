...
public class CheckOutProcess extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            if (customerBookingFound.get(0).isPaymentStatus() == true) {
                for (int i = 0; i < customerBookingFound.size(); i++) {
                    customerBookingFound.get(i).setDeletedFlag(false);
                }
                out.print("Check Out Completed For: " + customerBookingFound.get(0).getCustomer().getUsername());
                request.getRequestDispatcher("Home.jsp").include(request, response);
            } else {
                out.print("Customer have not make full payment");
                request.getRequestDispatcher("Home.jsp").include(request, response);
            }
